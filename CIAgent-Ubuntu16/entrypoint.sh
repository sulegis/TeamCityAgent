...
if [ -z "$TEAMCITY_NAME" ]; then
    echo "TEAMCITY_NAME variable not set, launch with -e TEAMCITY_NAME=NAME"
    exit 1
fi
...
if [ ! -d "$AGENT_DIR/bin" ]; then
	...
	echo "name=${TEAMCITY_NAME}" >> $AGENT_DIR/conf/buildAgent.properties
